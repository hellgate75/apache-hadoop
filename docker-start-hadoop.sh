...
if ! [[ -z "$HADOOP_CONFIG_TGZ_URL" ]]; then
  if ! [[ -e /root/hadoop_configured ]]; then
    mkdir -p /root/uploaded/config/hadoop
    echo "Download Hadoop configuration files from : $HADOOP_CONFIG_TGZ_URL  ..."
    curl -s $HADOOP_CONFIG_TGZ_URL | tar -xz -C /root/uploaded/config/hadoop/
